...
int main()
{
	string directory = "more_investigation_mesh/";
	...
	for (int m = 0; m < 1; m++)
	{
		...
		for (int i = 0; i < cloud_names.size(); i++)
		{
			...
			if (pcl::io::loadPLYFile("1.ply", *cloud) == -1) //* load the file
			{
			...
			cout << "L/W: " << dim_ratio_index[0] << " H/L: " << dim_ratio_index[1] << " H/D: " << dim_ratio_index[2] << " Average circularity: " << dim_ratio_index[3] << " Centroid length: " << dim_ratio_index[4] << " Max length index: " << dim_ratio_index[5] << endl << endl;
